...
import { BOOK_A_CALL_FORM_BG, bookACallHeroOverlapClasses, bookACallShellContainerClassName } from "@/data/bookACall";
import { clientMeetingWithAm3Booking, clientMeetingWithAm3Calendar } from "@/data/clientMeetingWithAm3";
import { cn } from "@/lib/utils";

const { formBubbleSrc, formBubbleAlt, womanImageSrc, womanImageAlt } = clientMeetingWithAm3Calendar;

/** GHL booking widget for client-meeting-with-am3 — same layout as Book a Call calendar section. */
export function ClientMeetingWithAm3CalendarSection() {
  return (
    <div
      className={`relative z-10 w-full overflow-x-hidden ${bookACallHeroOverlapClasses.formPull}`}
      aria-label="Schedule your client meeting with Rozita"
    >
      <div
                ...
                <div className="rounded-[18px] bg-white p-[3px] sm:rounded-[20px] sm:p-1">
                  <div className="overflow-hidden rounded-[15px] border-[3px] border-[#11104C]/12 bg-white px-4 py-8 sm:rounded-[17px] sm:px-6 sm:py-10 md:py-12 lg:border-[4px] lg:px-8 lg:py-12">
                    <GhlBookingEmbed
                      ...
                      config={{
                        ...clientMeetingWithAm3Booking,
                        scriptDatasetKey: "client-meeting-with-am3",
                      }}
                    />
                  </div>
                </div>
