...
type CallScheduledThankYouHeroProps = {
  headingId: string;
  archGradientId: string;
  title: string;
  children?: ReactNode;
  /** Optional content below the arch on #F5F8FF (e.g. hero video). */
  belowArch?: ReactNode;
};

/**
 * Shared thank-you hero — navy background, cyan glow, and gradient arch (Book a Call style).
 */
export function CallScheduledThankYouHero({
  headingId,
  archGradientId,
  title,
  children,
  belowArch,
}: CallScheduledThankYouHeroProps) {
  return (
    <section className="relative overflow-x-hidden overflow-y-visible" aria-labelledby={headingId}>
      <div className="relative z-20 bg-[#11104C] pb-0 pt-30 text-white sm:pt-32 lg:pt-[120px]">
        ...
        <Container className="relative z-20 max-w-[900px] px-6 text-center lg:px-20">
          <h1
            id={headingId}
            className="font-[var(--font-poppins)] text-[32px] font-bold leading-[1.15] text-balance text-white sm:text-[40px] lg:text-[48px]"
          >
            {title}
          </h1>
          {children}
      ...
      {belowArch ? (
        <div className="relative z-10 bg-[#F5F8FF] pb-12 pt-8 sm:pb-16 sm:pt-10 lg:pb-20">{belowArch}</div>
      ) : (
        <div className="relative z-10 bg-[#F5F8FF] pb-16 sm:pb-20 lg:pb-24" aria-hidden />
      )}
    </section>
  );
}
